...
# We need to make sure every `self.history.record(` is intercepted. 
# We'll replace it with a macro or helper.
# But wait, it's easier to just let `history.record` be. Since it takes an `EditCommand` and the current text, we can add `self.notify_timeline` by regexing.
# Wait, let's just use `re.sub` to replace `self.history.record(\n\s*(EditCommand::[^{]+\{\n(?:[^}]+)\}),\n\s*self.cursor.line,\n\s*self.cursor.col,\n\s*);`
# It's actually:
# self.history.record(
#     EditCommand::Insert {
#         pos,
...
redo_new = """EditCommand::Insert { pos, ref text } => {
                    self.buffer.insert(pos, text);
                    #[cfg(feature = "timeline")]
                    self.notify_timeline(TimelineOp::Insert { pos: *pos, text: text.clone() });
                    let end = pos + text.chars().count();
...
text = text.replace(redo_orig, redo_new)
